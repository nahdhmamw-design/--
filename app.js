...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  function renderList(filter=''){
    ...
    items.forEach(m=>{
      ...
      div.innerHTML = `<h4>${escapeHtml(m.trade_ar)} / ${escapeHtml(m.trade_en)}</h4><p>${escapeHtml(m.ingredient)} • ${escapeHtml(m.class)}</p>`;
  ...
  function showDetail(m){
    ...
    detailEl.innerHTML = `
      ...
      <p><strong>المادة الفعالة:</strong> ${escapeHtml(m.ingredient)}</p>
      <p><strong>الفئة:</strong> ${escapeHtml(m.class)}</p>
      <p><strong>جرعة عامة (بالبلوغ):</strong> ${escapeHtml(m.dose_adult || 'انظر النشرة')}</p>
      <p><strong>إرشاد آمن حسب العمر:</strong> ${escapeHtml(doseNote)}</p>
      <p><strong>المصدر:</strong> ${escapeHtml(m.source)}</p>
      <div style="margin-top:10px">
        <button class="btn" id="downloadPdf">تحميل كـ PDF</button>
      </div>
    `;
  ...
  function downloadPdf(m, age){
    const html = `
      ...
      <body><h2>${m.trade_ar} / ${m.trade_en}</h2>
